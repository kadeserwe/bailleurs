...
import sn.ssi.sigmap.config.KafkaProperties;
...
class BailleursKafkaResourceIT {
    ...
    void setup() {
        ...
        BailleursKafkaResource kafkaResource = new BailleursKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bailleurs-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bailleurs-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
